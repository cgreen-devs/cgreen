...
a complete source tree. *Cgreen* will also tally the ignored tests, so
it is visible when you run your tests.
...
`ignores`:: The number of tests that has been ignored by the `xEnsure` mechanism (see <<xensure>>)
