...
In C all function parameters are by value so if a function need to
return a value through a parameter that has to be done using a
pointer. Typically this is a pointer to the area or variable the
function should fill.

*Cgreen* provides `will_set_contents_of_parameter()` to handle this
...
When the mock for `convert_to_uppercase()` is called it will write the
string "UPPER CASE" in the area pointed to by `converted_string`.
