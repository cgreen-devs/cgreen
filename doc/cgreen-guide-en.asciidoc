//////////////////////////////////////////////////////////////////////
In this Asciidoc document we use the convention that one sentence is kept one a single line.
...
NOTE: The "boxed double" returned by `mock()` if instructed with `will_return_double()` have to be "unboxed", by the caller see <<double_mocks>> for details.

NOTE: The memory allocated to the copy of the struct returned using a pointer in `will_return_by_value` need to be deallocated by the caller of `mock()` or it will be lost. You can do this with the code in the `Box` example below.
...
But this would cause a memory leak since the area allocated by the `return_by_value()` macro is not deallocated.
This might not be a big problem, and you could make do with that simple version.
But to be sure we should free the allocated area.
It is referred to by the pointer returned from `mock()`.
So, here's a better, although a bit more complicated, version:
...
- Introduction of `will_return_by_value()`
- Introduction of `with_side_effect()`
