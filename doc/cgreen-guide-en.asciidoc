...
Marcus Baker <marcus@lastcraft.com>; Thomas Nilefalk <thomas@junovagen.se>; João Freitas <joaohf@gmail.com>;
v{VERSION}
...
*Cgreen* is designed to make it easy and natural to write assertions and
expectations. Many functions can be used for multiple data types, e.g.
`is_equal_to()` applies to all integer type values.

But the C language has its quirks. One of them is the fact that it is
impossible to inspect the datatypes of values during run-time. This has e.g.
forced the introduction of `is_equal_to_string()` to enable string comparisons.
...
But there is also the special assert
...
When you are using mocks with double typed values there are two different
issues to remember:

1. To use `double` as arguments to `mock()` you must "box" them
2. A `double` return value from `mock()` needs to be "unboxed"

So, what is this "boxing" and "unboxing" then?

Since a `double float` will not fit into the same memory space as
an integer *Cgreen* handles that by encapsulating ("boxing") the
`double` into an area which is represented by the pointer to it.
And that pointer can fit into the integer type value that *Cgreen*
...
So give a header file containg lines like
...
`cgreen-mocker` will, give that there are no errors, print something
...
WARNING: `cgreen-mocker` cannot manage the special handling that
`double` values require. (See <<double_mock>> for details.)
