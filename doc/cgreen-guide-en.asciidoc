...
|===========================================================================================
|*Macro*                                                      |*Description*
|`will_return(value)`                                         |`mock()` returns `value`, for integer types
|`will_return_by_value(struct, size)`                         |`mock()` returns a pointer to an allocated copy of the struct that can be copied and returned by value from the mocked function
|`will_return_double(value)`                                  |`mock()` returns `value`, for double floats (required because of C's type coercion rules which would otherwise convert a double into an int)
|`will_set_contents_of_parameter(parameter_name, pointer_to_value, size)`|Writes 'size' bytes from the pointed out value into where the referenced out (pointer) parameter is pointing
