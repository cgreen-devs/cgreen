...
When you use the BDD notation you can only have a single test subject (which is actually equivalent of a suite) in a single file because you can only have one `Describe()` macro in each file.
...
- `will_set_contents_of_output_parameter()` (and its older name `will_set_contents_of_parameter()`) now copies the source data at the time of the `expect()` call instead of storing a pointer to it. The source no longer has to remain valid until the mocked function is called.
