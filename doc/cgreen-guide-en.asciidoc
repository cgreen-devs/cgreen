...
This document will present *Cgreen* using the more modern and better BDD-style.
...
Particularly in conjuction with the BDD style test notation.
...
The `mock()` macro captures the parameter names and the `__func__` property (the name of the stub function).
...
Sometimes you need to set a field in a struct sent by reference to a mock function.
...
(except for `never_expect()` naturally).
...
To do this we should use another return macro, `will_return_by_value()`.
...
The underlying mechanism of this is that in the test we create the struct that we want to return, the macro `will_return_by_value()` copies that to a dynamically allocated area.
A pointer to that area will be returned by `mock()`.
...
This might be complicated, so *Cgreen* makes that a bit simpler.
...
But there are a lot of things going on behind the scenes, often with the help of some serious macro-magic, so special care needs to be taken when refactoring tests that have `expect()` in them.
...
So we don't have to make the local character variables in the test local, because those will remain live long enough.
